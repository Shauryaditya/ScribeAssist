...
const Alaysis = ({id, analysis, transcript }) => {

  const [clinicalLike, setClinicalLike] = useState(false);
  const [clinicalDislike, setClinicalDislike] = useState(false);
  const [billingLike, setBillingLike] = useState(false);
  const [billingDislike, setBillingDislike] = useState(false);
  const [review, setReview] = useState(false);
  const isLiked = clinicalLike ? "violet" : "white"
  const isDisliked = clinicalDislike ? "violet" : "white"
  ...
  const handleLikeDislike = async (isLike) => {
    try {
      ...
      if(updatedReviewValue){
        ...
        setClinicalLike(true);
        setClinicalDislike(false);
      }else{
        requestBody.clinicalDislike = true
        setClinicalDislike(true);
        setClinicalLike(false)
      }
  ...
  const handleLikeDislike2 = async (isLike) => {
    try {
      ...
      if(updatedReviewValue){
        ...
        setBillingLike(true);
        setBillingDislike(false);
      }else{
        requestBody.billingDislike = true
        setBillingDislike(true);
        setBillingLike(false)
      }
      console.log("Request body>>>>",requestBody);
