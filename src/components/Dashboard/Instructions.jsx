...
const Instructions = ({ instruction, id, transcript, instLike, instDislike }) => {
  ...
  const [Like, setLike] = useState(instLike)
  const [Dislike, setDislike] = useState(instDislike);
  const [isDisabled, setIsDisabled] = useState(false);
  const isLiked = Like ? "violet" : "white"
  const isDisliked = Dislike ? "violet" : "white"

  useEffect(() => {
    // Initialize state variables with data from props
    setUpdatedInstructions(instruction);
    if(instLike || instDislike){
      setIsDisabled(true);
    }else{
      setIsDisabled(false);
    }
  ...
  const handleLikeDislike = async (isLike) => {
    try {
      ...
      if(updatedReviewValue){
        requestBody.instLike = true
        setLike(true);
        setIsDisabled(true);
      }else{
        requestBody.instDislike= true
        setDislike(true);
        setIsDisabled(true);
      }
